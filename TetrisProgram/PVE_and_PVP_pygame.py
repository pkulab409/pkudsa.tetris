...
class Wall():
    ...
    def __init__(self):
        # 颜色矩阵，底板矩阵
        self.screen_color_matrix = [[None] * GRID_NUM_WIDTH for i in range(GRID_NUM_HEIGHT)]
        self.now_cube = None
        pass
    ...
    def draw_matrix(self):    #绘制色块
        for i, column in zip(range(GRID_NUM_WIDTH), self.screen_color_matrix):
    ...
    def drawNowBrick(self):
        if self.now_cube is not None:
            self.now_cube.drawNow()
...
w =  Wall()
if team1 == "person":
    agame.player[0].set_attrs(w, agame, screen)
if team2 == "person":
    agame.player[1].set_attrs(w, agame, screen)
...
while agame.state == 'gaming':
    ...
    if not pause:
        # 直接从列表中获取block，为了让玩家操作时能正确绘图，调整了一下顺序
        block = agame.pack.get(agame.time + 1) - 1
        w.now_cube = Brick(block)
        agame.turn()
        w.screen_color_matrix = agame.visualBoard.list
        point1 = agame.point1
        point2 = agame.point2
        combo = agame.combo
